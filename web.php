...
if (version_compare(PHP_VERSION, '5.4.7', '<')) {
    exit('miniProxy requires PHP version 5.4.7 or later.');
}
$requiredExtensions = ['curl', 'mbstring', 'xml'];
foreach ($requiredExtensions as $requiredExtension) {
    if (! extension_loaded($requiredExtension)) {
        exit("miniProxy requires PHP's \"".$requiredExtension.'" extension. Please install/enable it on your server and try again.');
    }
}
...
function makeRequest($url)
{
    ...
    $removedHeaders = removeKeys($browserRequestHeaders, [
...
if (empty($scheme)) {
    ...
    exit('Error: Detected a "'.$scheme.'" URL. miniProxy exclusively supports http[s] URLs.');
}
...
if (! $urlIsValid) {
    exit('Error: The requested URL was disallowed by the server administrator.');
}
